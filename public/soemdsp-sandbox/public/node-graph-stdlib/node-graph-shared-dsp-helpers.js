...
/**
 * Ladder face + DSP coefficients. Must live in always-loaded stdlib — release
 * shells do not load ladder-filter-live-evaluator.js (native-only twins).
 */
function nodeGraphLadderFilterCoefficients(
  frequency, resonance, mode, stages, sampleRate, runtime = null, nodeId = "", state = null,
) {
  const rate = Math.max(1, Number(sampleRate) || (typeof nodeGraphMvp !== "undefined" ? nodeGraphMvp.sampleRate : 0) || 44100);
  const rawFreq = typeof nodeGraphSafeFilterNumber === "function"
    ? nodeGraphSafeFilterNumber(frequency, runtime, nodeId, state, "ladder filter frequency")
    : Number(frequency);
  const frequencyValue = Math.max(0, Number.isFinite(rawFreq) ? rawFreq : 0);
  const safeFrequency = clampNodeSliderValue(frequencyValue, 0.000001, Math.min(20000, rate * 0.49));
  const rawRes = typeof nodeGraphSafeFilterNumber === "function"
    ? nodeGraphSafeFilterNumber(resonance, runtime, nodeId, state, "ladder filter resonance")
    : Number(resonance);
  const feedback = clampNodeSliderValue(Number.isFinite(rawRes) ? rawRes : 0, 0, 0.999);
