...
function nodeGraphClipperLimiterPrep(input, minDb = -12, maxDb = 0, gainDb = 0) {
  ...
  const drive = nodeGraphClipperLimiterDbToLin(nodeGraphFiniteNumber(gainDb));
  const x = (nodeGraphFiniteNumber(input)) * drive;
  const ax = Math.abs(x);
  if (ax <= minLin) {
    return { dry: true, y: x };
  }
...
function nodeGraphClipperLimiterFinish(prep, shaped) {
  ...
  return prep.sign * (prep.minLin + (nodeGraphFiniteNumber(shaped)));
}
...
function nodeGraphClipperLimiterFrame(mono, left, right, minDb, maxDb, gainDb, state = null, oversample = 2) {
  ...
  return {
    ...
    Left: nodeGraphClipperLimiterSample((nodeGraphFiniteNumber(left)) + m, minDb, maxDb, gainDb, st?.left, oversample),
    Right: nodeGraphClipperLimiterSample((nodeGraphFiniteNumber(right)) + m, minDb, maxDb, gainDb, st?.right, oversample),
  };
}
