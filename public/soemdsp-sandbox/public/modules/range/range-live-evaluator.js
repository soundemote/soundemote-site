...
nodeGraphLiveModuleEvaluators.range = ({ runtime, node, nodeId, frame, frames, frameValues, mixInput }) => {
  ...
  const outLow = readNodeGraphLiveEffectiveParam(runtime, node, "outLow", -10, frame, frames, frameValues);
  const outHigh = readNodeGraphLiveEffectiveParam(runtime, node, "outHigh", 10, frame, frames, frameValues);
